...
namespace Core.Camera.Editor
{
	...
	public class CameraRigEditor : UnityEditor.Editor
	{
		...
		void OnSceneGUI()
		{
			...
			var fmh_42_60_639083513720208465 = Quaternion.LookRotation(Vector3.up); Vector3 bottom = Handles.FreeMoveHandle(bottomPosition, size, snap,
			                                        Handles.RectangleHandleCap);
			var fmh_44_54_639083513720224271 = Quaternion.LookRotation(Vector3.up); Vector3 top = Handles.FreeMoveHandle(topPosition, size, snap,
			                                     Handles.RectangleHandleCap);
			var fmh_46_56_639083513720225283 = Quaternion.LookRotation(Vector3.up); Vector3 left = Handles.FreeMoveHandle(leftPosition, size, snap,
			                                      Handles.RectangleHandleCap);
			var fmh_48_58_639083513720225979 = Quaternion.LookRotation(Vector3.up); Vector3 right = Handles.FreeMoveHandle(rightPosition, size, snap,
			                                       Handles.RectangleHandleCap);
